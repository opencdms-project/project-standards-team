...
C
      INTEGER*2 IRGB(3)
C
      IF (DEVMODE.EQ.AQCMODE) THEN
C          .. EGA SCREEN -- SPECIAL CASE
         DO 15 J=1,16
            CALL INT2RGB(PALCLR(J),IRGB)
            CALL RGB2EGA(IRGB,CLRVAL(J,1))
            CALL REPPAL(CLRVAL(1,1))
   15    CONTINUE         
      ELSE
C      
...
C
      IF (DEVMODE.EQ.AQCMODE) THEN
C          .. EGA SCREEN -- SPECIAL CASE
         CALL RGB2EGA(IRGB,ICLRNRM(1))
         CALL SETXPA(INDEX,ICLRNRM(1))
      ELSE   
C     
         NRMFAC = NRMVAL(1)/3
         DO 10 I=1,NTYP
