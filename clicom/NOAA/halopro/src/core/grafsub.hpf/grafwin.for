$STORAGE:2
      SUBROUTINE GRAFWIN(IWINFLG,XMIN,YMIN,XMAX,YMAX)
C
C       ** OBJECTIVE:  OPEN OR CLOSE A WINDOW ON A HALO GRAPHICS SCREEN
C                      THE VALUE OF INPUT VARIABLE IWINFLG DETERMINES WHETHER
C                      THE SCREEN BEHIND THE WINDOW WILL BE SAVED
C       ** INPUT
C            IWINFLG..... 1=OPEN A WINDOW, SAVE TEXT BEHIND WINDOW
...
C
      IF (ICNTRL.GT.0) THEN
C
C          ** IF BUFFER SIZE(I*4) EQUALS 1 (4BYTES) THERE IS NO ROOM TO SAVE 
C             THE SCREEN BEHIND THE WINDOW.  SET FLAG SO WINDOW IS NOT SAVED.
         IF (MAXBYT.LE.4) ICNTRL=2
C
...
C
   ...
   25       CONTINUE   
            ...
            IF (ENDBUF(IWINDOW).GT.MAXBUF) THEN
               ...
               CALL GETMSG(554,MSG)
C **DEBUG
               WRITE(MSG,'(A,I15)')'WINDOW TOO BIG.  SIZE=',
     +                            ENDBUF(IWINDOW)
