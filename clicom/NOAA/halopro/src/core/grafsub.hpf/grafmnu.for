...
C
      IF (WINSTAT.NE.0 .AND. WINSTAT.NE.3) THEN
...
C
C       **INPUT:
C            WINSTAT....FLAG TO INDICATE METHOD OF PROCESSING WINDOW AND MENU
C
C                              SAVE    DRAW  GET     REMOVE  RESTORE  INITIAL
C                     WINSTAT  SCREEN  MENU  CHOICE  MENU    SCREEN   CURSOR
C                     -------  ------  ----  ------  ------  -------  -------
C                        0       -      -      -       X        ?        -
C                        1       X      X      X       X        X        1
C                        2       -      X      X       -        -        1
...
C            XWIN,YWIN..LOWER LEFT WINDOW CORNER IN NORMALIZED DEVICE COORDS
C            HELPLVL....THE GRAPHICS HELP LEVEL NUMBER FOR THIS MENU
C            OUTCHAR....INITIAL CURSOR POSITION; USED ONLY IF WINSTAT=4,5 OR 6
C       ** OUTPUT:
C            OUTCHAR....THE CHARACTER*2 CHOICE RESULT
C                       CHOICE NUMBERS ARE LEFT JUSTIFIED
C
...
C
     ...
     +           XOLD(MXMNU),YOLD(MXMNU),ASPMNU(MXMNU),XHELP,YHELP
...
C     
      LOGICAL       FIRSTCALL
      DATA    FIRSTCALL /.TRUE./
      DATA    ISPFAC /.8/,XTRA_X/.01/
C **DEBUG
C         WRITE(999,*)'+++++++++++++++++BEG ARGMNU++++++++++++++++'
C
...
C
      ...
      TXTHTND = SYSHTND
C
      IF (WINSTAT.EQ.6) THEN
         IWSTAT =2
         ICURFLG=1
         IWCLOS =1
      ELSE IF (WINSTAT.EQ.5) THEN
         IWSTAT=1
         ICURFLG=1
         IWCLOS =1
...
C      
         ...
         DO 110 J = 1,NUMCHOICE+1
            CALL INQSTS(ITEMS(J),HEIGHT,CWIDTH,OFFSET)
            IF (CWIDTH.GT.WIDTH) THEN
               WIDTH = CWIDTH
               JSAV = J
            END IF
110      CONTINUE
         CALL INQSTS(ITEMS(JSAV),HEIGHT,WIDTH,OFFSET)
         XPOS = WIDTH 
         YPOS = (HEIGHT+ISPFAC*OFFSET)*(NUMCHOICE+3) + HEIGHT
         CALL MAPWTN(XPOS,YPOS,XWN,YWN)
         CALL INQVIE(XNDUL,YNDUL,XNDLR,YNDLR)
C **DEBUG
        WRITE(999,*)'+++++ ARGMENU AFT STMT 110 +++++++++++++++++++'
C **DEBUG         
         CALL INQVIE(XNDUL,YNDUL,XNDLR,YNDLR)
         CALL INQWOR(XWLL,YWLL,XWUR,YWUR)
         CALL MAPNTD(XNDUL,YNDUL,IXDUL,IYDUL)
         CALL MAPNTD(XNDLR,YNDLR,IXDLR,IYDLR)
         CALL INQSTS(ITEMS(JSAV),HEIGHT,WIDSTR,OFFSET)
         XRATIO = (IXDLR-IXDUL+1)/(XWUR-XWLL)
         YRATIO = (IYDLR-IYDUL+1)/(YWUR-YWLL)
         IWIDSTR = WIDSTR * XRATIO
         IHGHT   = HEIGHT * YRATIO
         WRITE(999,*) '------------- ARGMENU -- STRING=',ITEMS(JSAV)
         WRITE(999,*)'HGHT,WID,IHGHT,IWID=',HEIGHT,WIDSTR,IHGHT,IWIDSTR
         WRITE(999,*)'-------------- END RATIO CALC'
         CALL MAPWTD(XWLL,YWLL,IXDUL,IYDLR)
         CALL MAPWTD(XWLL+WIDSTR,YWLL+HEIGHT,IXD,IYD)
         IWIDSTR = IXD-IXDUL
         IHGHT = IYDLR-IYD
         WRITE(999,*)'HGHT,WID,IHGHT,IWID=',HEIGHT,WIDSTR,IHGHT,IWIDSTR
         WRITE(999,*)'-------------- END MAP CALC'
        WRITE(999,*)'+++++ YPOS,XWN,YWN=',YPOS,XWN,YWN
C **END DEBUG        
         XWN = XWN + .02 + XTRA_X
         YWN = (YNDLR-YWN) + .02
C **DEBUG
        WRITE(999,*)'+++++ADJ XWN,YWN=',XWN,YWN
C **END DEBUG        
C
...
C
         ...
         YWIN2(IMENU) = YWIN
C **DEBUG
         WRITE(999,*)'+++++ MAX WIDTH,HT,ADJASP=',WIDTH,HEIGHT,ADJASP
         WRITE(999,*)'+++++ XWIN1,XWIN2=',XWIN1(IMENU),XWIN2(IMENU)
         WRITE(999,*)'+++++ YWIN1,YWIN2=',YWIN1(IMENU),YWIN2(IMENU)
C ** END DEBUG
...
C
         ICLR  = 1
         CALL DFWINASP(ITEMS(JSAV),IDFONT,ICLR,TXTHTND,
     +                 XTRA_X,ADJASP,WINASP)
         ASPMNU(IMENU) = WINASP   
C **DEBUG         
         CALL INQVIE(XNDUL,YNDUL,XNDLR,YNDLR)
         CALL INQWOR(XWLL,YWLL,XWUR,YWUR)
         CALL INQSTS(F1LINE,HEIGHT,WIDSTR,OFFSET)
         CALL MAPWTD(XWLL,YWLL,IXDUL,IYDLR)
         CALL MAPWTD(XWLL+WIDSTR,YWLL+HEIGHT,IXD,IYD)
         IWIDSTR = IXD-IXDUL
         IHGHT = IYDLR-IYD
         WRITE(999,*) '------------- ARGMENU -- STRING=',F1LINE
         WRITE(999,*)'HGHT,WID,IHGHT,IWID=',HEIGHT,WIDSTR,IHGHT,IWIDSTR
         WRITE(999,*)'-------------- END MAP CALC'
C **END DEBUG        
C      
         CALL INQSTS(ITEMS(JSAV),HEIGHT,CWIDTH,OFFSET)
         HTLIN = HEIGHT + ISPFAC*OFFSET
C **DEBUG
         WRITE(999,*)'+++++ MAX WIDTH,HT,WINASP=',CWIDTH,HEIGHT,WINASP
C **END DEBUG            
         CALL INQSTS(ITEMS(1),HEIGHT,CWIDTH,OFFSET)
...
180      CONTINUE
C
C          .. DISPLAY THE F1 AND ESC KEY DEFINITIONS
C
C         YPOS = YHIGH - (NUMCHOICE+2)*HTLIN
         YPOS = YPOS - HTLIN
         CALL MOVTCA(XPOS,YPOS)
         CALL SETSTC(1,1)
         CALL STEXT(F1LINE)
C         YPOS = YHIGH - (NUMCHOICE+3)*HTLIN
         YPOS = YPOS - HTLIN
         CALL MOVTCA(XPOS,YPOS)
         CALL DELTCU
         CALL STEXT(ESCLINE)
...
C
      IF (IWSTAT.GT.0) THEN
         ...
         END IF
         CALL PIKITEM(HTLIN,NUMCHOICE,HELPLVL,XHELP,YHELP,OUTCHAR)
         SAVCHOI(IMENU) = OUTCHAR
      ENDIF   
C
C   RESTORE THE WINDOW AND CURSOR POSITION AND EXIT.
C
C      IF (IWSTAT.LE.1) THEN
      IF (IWCLOS.EQ.1) THEN
         CALL SETXOR(0)
