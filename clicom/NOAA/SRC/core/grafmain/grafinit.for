...
C
      ...
      CHARACTER*1 NULL
      INTEGER*2 MSGNBR(7)
C       .. MINIMUM ARRAY SIZE FOR BUFFER -- REQUIRED BY WNDSMRY       
      INTEGER*4 MXI4WRK
      PARAMETER (MXI4WRK=51*16*13) 
      REAL*4 HLDARRAY(NRCD),LATLIST(MAXROW)
     +      ,LONLIST(MAXROW)
      CHARACTER*12 ROWHDR(MAXROW)
      CHARACTER*8 STNLIST(MAXROW)
      COMMON /SCRATCH/ HLDARRAY,LATLIST,LONLIST,ROWHDR,STNLIST  
...
C 
      ...
      IF (IGRAPH.EQ.2) THEN
         CALL SELAREA(BUFFER,IBUFD,LOWLAT,HILAT,LOWLON,HILON,STNLIST,
     +                LATLIST,LONLIST,MXDATROW,RTNFLAG)
         IF (RTNFLAG.EQ.'4F') THEN
            GO TO 20 
         END IF
         CALL LOCATE(9,1,IERR)
