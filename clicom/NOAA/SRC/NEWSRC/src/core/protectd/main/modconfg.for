...
C
     ...
     +         ,FORM='FORMATTED')
         DO 100 I = 1,100
            ...
            CALL LOW2UP(LINE)
C
C     DETERMINE LEADING BLANKS OF TEXT (LEFT JUSTIFY). IF FOUND SKIP IT
C
            DO 50 K=1,79
               IF (LINE(1:K).NE.' ')THEN
                  L = K
...
C
         ...
         DO 150 I = 1,100
            LINE = TEXT(I)
            CALL SEARCH(I,LINE,'REM     ',3,NBREM,J)
            IF (NBREM .EQ. 0 ) THEN
               J = 0
               CALL SEARCH(I,LINE,'SHELL   ',5,SHELL,J)
               CALL SEARCH(I,LINE,'ANSI.SYS',79,ANSI,J)
               CALL SEARCH(I,LINE,'LASTDRIV',8,LASTDR,J)
               CALL SEARCH(I,LINE,'FCBS    ',4,FCBS,J)
               CALL SEARCH(I,LINE,'FILES   ',5,FILES,J)
               CALL SEARCH(I,LINE,'BUFFERS ',7,BUFFER,J)
            ELSE
               NBREM = 0
            ENDIF
  150    CONTINUE
...
C 
         DO 270 I = 1,100
            ...
            IF (I.EQ.SHELL) THEN
               I1 = 0
               CALL SEARCH(I,LINE,'COMMAND ',79,I1,J)
               IF (I1.NE.I) THEN
                  MSG = ' '
                  IR = IR + 1
                  CALL SCRNMSGI(MSG,IR,14)
...
$PAGE
************************************************************************
      SUBROUTINE SEARCH(I,LINE,TEXT,LENSTR,I1,JCOL)
C
...
C   LENSTR..LENGTH OF STRING TO BE SEARCH
