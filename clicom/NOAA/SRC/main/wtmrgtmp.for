...
C     PROGRAM WTMRGTMP
C
C     ** OBJECTIVE:  PROGRAM CONSTRUCTS A TEMPORARY MERGESEL.xxx FILE 
C                    BASED ON THE DATA-TYPE OF THE FILE TO BE MERGED.  IT  
...
C
     ...
     +           'UPPER-AIR DATA','NORMALS'/
      DATA TBLLIN1/33,33,33,33,33,33,33,33/
C
      NULL = CHAR(0)
      CTRLZ = CHAR(26)
      CHRRTN = CHAR(13)
...
C          MERGESEL.xxx AND THE OUTPUT FILE MERGESEL.TMP -- NOTE THAT THE 
C          OUTPUT FILE IS OPENED AS BINARY SO THAT THE PROGRAM HAS CONTROL 
C          OF ALL CHARACTERS SUCH AS LINE FEED AND END OF FILE MARKS.  EARLY 
C          VERSIONS OF COSORT ARE VERY PARTICULAR ABOUT THE FILE CONTENTS.
C
      INFIL = '\COSORT\MERGESEL.'//RECTYPE(1:3)
      OPEN (63,FILE=INFIL,STATUS='OLD',FORM='FORMATTED',IOSTAT=IOCHK)
      IF (IOCHK.NE.0) THEN
C          .. ERROR OPENING INPUT FILE MERGESEL.xxx
         GO TO 901
      ENDIF         
      OUTFIL = 'MERGESEL.TMP'
      OPEN (64,FILE=OUTFIL,STATUS='UNKNOWN',FORM='BINARY')
C
C       ** GET THE LINE NUMBER OF THE REFERENCE TO THE DATAEASE FILE NAME IN
C          MERGESEL.xxx
C
      LIN1 = TBLLIN1(ITYP)
C
C       ** COPY THE FILE MERGESEL.xxx TO MERGESEL.TMP REPLACING THE DEFAULT
C          REFERENCES TO THE DATAEASE FILE NAME WITH THE ACTUAL VALUE FOR 
C          THE FILE NAME
C
      NLIN = LIN1-1
      CALL CPYLIN(NLIN)
